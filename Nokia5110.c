...
void static lcdwrite(enum typeOfWrite type, char message){
  while((SSI0_SR_R & 0x10));
	GPIO_PORTA_DATA_R &= ~0x40; 		// Set PA6 (D/C pin) to low state
	GPIO_PORTA_DATA_R |= (type << 6);
	SSI0_DR_R = message;
	while((SSI0_SR_R & 0x10));
}
...
void Nokia5110_Init(void){
  ...
  GPIO_PORTA_DEN_R |= 0xFF;	         // Enable Digital Pins PA0-7
  GPIO_PORTA_DIR_R |= 0xC0;	         // PA6,7 output GPIO
	GPIO_PORTA_DIR_R &= ~0x03;				 // PA0,1 Input GPIO
	
	SYSCTL_RCGCSSI_R |= 0x01;            // Enable and provide clock for SSI0
	while(!(SYSCTL_RCGCSSI_R & 0x01));   // Wait for module to initialize
	SYSCTL_RCGCGPIO_R |= 0x01;         // Enable clock for GPIO Port A
	while(!(SYSCTL_RCGCGPIO_R & 0x01));// Wait for Port A to initialize
	GPIO_PORTA_AFSEL_R |= 0x3C;        // Enable Alternate Function for PA2-5
	GPIO_PORTA_PCTL_R = (GPIO_PORTA_PCTL_R & 0xFF0000FF) + 0x00222200; 	 			// Enable correct PCTL (For SSI0)
	GPIO_PORTA_PUR_R |= 0x3C;          // Enable Pullup resistors PA2-5
	
	SSI0_CR1_R &= ~0x02;               // Disable SSI Port for configuration
	SSI0_CR1_R = 0x00;							   // Configure SSI0 as master 
	
	SSI0_CC_R = 0x00;
	SSI0_CPSR_R = 2;
	SSI0_CR0_R &= 0x00;
	SSI0_CR0_R =   (2U << 8);  				 // SCR = 2
  SSI0_CR0_R |=  (0x7 << 0); 				 // 8 bit data size
  SSI0_CR1_R |=  (1U << 1);  				 // Enable SSI
	
	GPIO_PORTA_DATA_R |= 0x80;				 // Set PA7 (Rst pin) high
	GPIO_PORTA_DATA_R &= ~0x80;        // Set rst pin low
	for(delay = 0; delay < 100; delay++); // Delay for rst pulse
	GPIO_PORTA_DATA_R |= 0x80;         // Rst pin high again
	
	lcdwrite(COMMAND,0x21);
	lcdwrite(COMMAND,0xA8);
